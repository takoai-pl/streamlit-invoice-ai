...
from frontend.data.models import InvoiceModel, ProductModel, BusinessModel, ClientModel
...
class APIProvider:
    ...
    def invoice_get(self, invoice_no: str, language: str) -> InvoiceModel:
        ...
        response = requests.get(f"{self.base_url}/invoice/{encoded_invoice_no}/{language}")
    ...
    def business_add(self, business) -> None:
        requests.post(f"{self.base_url}/business", json=business.to_json())

    def business_put(self, business) -> None:
        requests.put(f"{self.base_url}/business/{business.name}", json=business.to_json())
    ...
    def client_put(self, client) -> None:
