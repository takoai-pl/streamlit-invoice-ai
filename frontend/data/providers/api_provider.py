...
class APIProvider:
    ...
    def business_get(self, business_name: str) -> BusinessModel:
        response = requests.get(f"{self.base_url}/business/{business_name}/", headers=self.headers)
        response.raise_for_status()
    ...
    def business_del(self, business_name: str) -> None:
        response = requests.delete(
            f"{self.base_url}/business/{business_name}/",
            headers=self.headers
        )
        response.raise_for_status()
    ...
    def client_get(self, client_name: str) -> ClientModel:
        response = requests.get(f"{self.base_url}/client/{client_name}/", headers=self.headers)
        response.raise_for_status()
    ...
    def client_del(self, client_name: str) -> None:
        response = requests.delete(
            f"{self.base_url}/client/{client_name}/",
            headers=self.headers
        )
        response.raise_for_status()
